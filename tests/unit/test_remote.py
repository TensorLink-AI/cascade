...
def _receipt_dict(role="king", uid=0, hotkey="hk"):
    return {
        "miner_hotkey": hotkey, "miner_uid": uid, "role": role, "gen_cid": CID_A,
        "trained_pointer": format_trained_pointer(CID_T), "corpus_digest": "d",
        "train_block": 10, "tar_digest": "t",
    }
...
def test_parse_receipt_extracts_json_after_sentinel():
    ...
    assert got["role"] == "king" and got["gen_cid"] == CID_A
...
def test_dispatch_returns_entry_on_success():
    ...
    entry = disp.dispatch(_host(), gen_cid=CID_A, uid=0, hotkey="hk", role="king",
                          base_seed=1, block=10)
    assert entry.role == "king" and entry.trained_pointer == format_trained_pointer(CID_T)
...
def test_dispatch_raises_on_nonzero_rc():
    ...
    with pytest.raises(RemoteDispatchError):
        disp.dispatch(_host(), gen_cid=CID_A, uid=0, hotkey="hk", role="king",
                      base_seed=1, block=10)
...
def test_load_hosts_parses_toml(tmp_path):
    ...
    p.write_text(
        '[[host]]\nname="king"\nhost="10.0.0.1"\ncuda_device="0"\n'
        'forward_env=["IPFS_NODE_URL"]\n\n'
        '[[host]]\nname="chal"\nhost="10.0.0.2"\nport=2200\n',
        encoding="utf-8",
    )
    hosts = load_hosts(p)
    assert [h.name for h in hosts] == ["king", "chal"]
    assert hosts[0].cuda_device == "0" and hosts[0].forward_env == ("IPFS_NODE_URL",)
...
class _RecordingDispatcher:
    ...
    def dispatch(self, host, *, gen_cid, uid, hotkey, role, base_seed, block):
        _RecordingDispatcher.calls.append((host.name, role, gen_cid))
...
def test_run_round_remote_assigns_king_and_challenger_to_separate_hosts(cfg, tmp_path, monkeypatch):
    ...
    by_role = {role: name for (name, role, _cid) in _RecordingDispatcher.calls}
...
def test_run_round_remote_aborts_when_king_fails(cfg, tmp_path, monkeypatch):
    ...
    commits = [Commitment(uid=0, hotkey="a", coldkey=None,
                          payload=f"metro-v1:gen:hippius:{CID_A}", commit_block=5)]
