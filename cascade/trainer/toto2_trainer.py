...
def _lr_at(token_pos: int, total: int, warmup: int, base_lr: float, *,
    ...
    """LR at ``token_pos`` under the contract's ``lr_schedule``.

    ``warmup_cosine`` — linear warmup then cosine to 0 over the round's token
    budget. Right for a self-contained from-scratch round; wrong for a
    ...
    belongs to cutting a release checkpoint, not to the round loop.
    """
    if schedule == "wsd":
...
class Toto2Trainer:
    ...
    def train(
        ...
        *,
        ...
        if schedule not in ("warmup_cosine", "wsd"):
            raise ValueError(
                f"unknown [training] lr_schedule {schedule!r}; expected "
                "'warmup_cosine' or 'wsd'"
            )
