...
def contract_payload(contract: object) -> dict:
    ...
    (DEC-CA-0029): the trainer ships the body in the signed manifest, and a
...
# ── the locked (consensus) projection of a training contract — DEC-CA-0029 ───
...
#
# The three that stay locked are exactly the terms this validator CONSUMES with
# its own local value (grep cfg.training in cascade/validator — there is nothing
# else): ``base_arch_digest`` gates the architecture the eval assumes,
# ``arch_preset`` names the sizes entries are bucketed into, and ``expected_gpu``
# is the matched-hardware pin — which must come from local config, since a pin
# read out of the declaration the trainer wrote would gate the trainer against
# itself.
LOCKED_CONTRACT_FIELDS = ("arch_preset", "base_arch_digest", "expected_gpu")
...
class TrainingManifest:
    ...
    # (DEC-CA-0029). Exactly the payload ``contract_digest`` hashes, so
    ...
    def canonical_body(self) -> bytes:
        ...
        # Same drop-when-unset convention (DEC-CA-0029): a manifest from a
