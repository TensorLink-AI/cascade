#!/bin/bash
# One-shot validator relaunch — exact prior arg line (2026-08-21 v0.6.0 go-live).
# Live copy on the box: /root/cascade/restart_validator.sh (replace on deploy).
#
# Hardened after the r47 stray-trio incident (OPSLOG 2026-08-28 12:35): a
# stale validator (pid from Aug 20!) survived the 07:41 flip because the pkill
# pattern only matched one cmdline shape. See restart_trainer.sh for the full
...
#   DRY_RUN=1 ./restart_validator.sh    or    ./restart_validator.sh --dry-run
cd /root/cascade || exit 1

SERVICE="cascade-validator"
LAUNCH_DESC=".venv/bin/python .venv/bin/cascade-validator --wallet-name main_one ..."
# Console entrypoint under any path prefix + python -m module forms.
PATTERNS=(
  'cascade-validator'
  '-m cascade\.validator(\.main)?([[:space:]]|$)'
)
...
set -a && . ./.env && set +a
nohup .venv/bin/python .venv/bin/cascade-validator \
  --wallet-name main_one --wallet-hotkey sn_validator_hk \
  --network finney --chain-toml chain.toml \
  --device cpu --eval-hosts eval_hosts.toml \
  >> validator.log 2>&1 &
NEW_PID=$!
echo "validator pid $NEW_PID"
...
if ! kill -0 "$NEW_PID" 2>/dev/null; then
  echo "FATAL: new $SERVICE (pid $NEW_PID) exited right after launch — check validator.log"
  exit 1
fi
