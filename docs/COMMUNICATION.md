...
| 8e | Eval-pool composition | owner pool publisher → dashboards, miners | `status/pool.json` (manifest bucket): rolling per-snapshot summaries — `effective_block`, data cutoff, tar sha256, series counts per domain × granularity; AGGREGATE shape only, never series identities (see `cascade.shared.pool_status`) | unsigned, presentational only — the signed manifest's `eval_pool_key`/`eval_pool_sha256` pin remains the audit record | single writer (the daily `cascade-pool publish` cron); consumers map a round to its snapshot by the same greatest-`effective_block`-≤-epoch-block rule validators select by |
