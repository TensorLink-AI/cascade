...
- **Restart after any announced `[training]` change.** The contract digest is
  ...
  *Once `[scoring] declared_contract_from_block` is armed (DEC-CA-0029) this
  ...
  actually consume — `base_arch_digest`, `arch_preset`, `expected_gpu`, and
  each extra size's GPU pin. A worker-image re-pin, an optimizer or schedule
  change, a budget change: your validator keeps scoring straight through, no
  restart, no announcement to wait on. Run `cascade-audit` as usual — a round
  trained under terms your `chain.toml` doesn't describe shows up as a
  `contract-declaration` WARN listing every field that moved, which is the
...
| `rejecting manifest … contract_locked_mismatch` | a LOCKED contract term (arch, size, GPU pin) differs — pull + restart; unlike the declared terms these are never waved through |
