...
**Stage 1 — shadow scratch control (build next; no consensus change).** Every
M rounds (start M=4) the trainer additionally trains a FROM-SCRATCH model on
the reigning king's generator under the identical contract, benches it on the
three suites, and publishes the numbers as telemetry beside the bench report.
No throne, no weights, no envelope change; cost ~1/M of one lane. Output is
the two curves every later stage depends on: lineage-vs-scratch gap, and its
trend (compounding / flat / closing).
