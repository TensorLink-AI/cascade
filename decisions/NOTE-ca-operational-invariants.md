...
- `[training]` edits change `contract_digest` → the VALIDATOR must restart
  ...
  boundary. NARROWED by [[DEC-CA-0036]] once `[scoring]
  declared_contract_from_block` is armed: only the LOCKED terms
  (`base_arch_digest`, `arch_preset`, `expected_gpu`, `train_seed_salt`,
  per-size pins) still force a fleet restart — every other `[training]` edit
  is trainer-declared and ships alone. Until it is armed (it ships at 0), the
...
- A validator restart is for an ACTUAL SCORING CHANGE. `[scoring]`, `[eval]`,
  ...
  reach for the [[DEC-CA-0036]] declared path to dodge that window — it
